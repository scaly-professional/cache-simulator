 ...
 * @author Eric Gan <ehgan@andrew.cmu.edu>
 */
...
static void trans_32_1024(size_t M, size_t N, const double A[N][M],
                          double B[M][N], double tmp[TMPCOUNT]) {
    assert(M > 0);
    assert(N > 0);
    for (size_t i = 0; i < N; i += 8) {
        for (size_t j = 0; j < M; j += 8) {
            for (size_t ib = i; ib < i + 8; ib++) {
                for (size_t jb = j; jb < j + 8; jb++) {
                    B[jb][ib] = A[ib][jb];
                }
            }
        }
